...
| `MODEL_IDLE_TIMEOUT` | `0` | Seconds to keep models loaded after last task (0 = immediate offload). |
| `AGGRESSIVE_OFFLOAD` | `true` | Immediately unload models when idle (overridden by `MODEL_IDLE_TIMEOUT`). |
| **Optimization** | | |
