...
mock_whisperx.assign_word_speakers.return_value = {"segments": [{"start": 0.0, "end": 1.0, "text": "hello", "speaker": "SPEAKER_00"}]}
...
def test_run_diarization_duration_warning(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch("modules.inference.pipeline.diarization.logger") as mock_logger:
        with mock.patch("modules.inference.pipeline.diarization._get_align_model", return_value=(mock.MagicMock(), {})):
            with mock.patch("modules.inference.pipeline.diarization._get_diarize_pipeline") as mock_dp:
